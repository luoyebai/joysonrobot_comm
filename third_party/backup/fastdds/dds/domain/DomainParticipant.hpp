...
class DomainParticipant : public Entity {
   public:
     ...
     * @warning Do not call this method from a \c DomainParticipantListener callback.
     */
    RTPS_DllAPI ReturnCode_t set_listener(DomainParticipantListener* listener, const std::chrono::seconds timeout);
     ...
     * @warning Do not call this method from a \c DomainParticipantListener callback.
     */
    RTPS_DllAPI ReturnCode_t set_listener(DomainParticipantListener* listener, const StatusMask& mask);
     ...
     */
    RTPS_DllAPI Publisher* create_publisher(const PublisherQos& qos, PublisherListener* listener = nullptr,
     ...
     * @return Pointer to the created Publisher.
     */
    RTPS_DllAPI Publisher* create_publisher_with_profile(const std::string& profile_name,
     ...
     */
    RTPS_DllAPI Subscriber* create_subscriber(const SubscriberQos& qos, SubscriberListener* listener = nullptr,
                                              const StatusMask& mask = StatusMask::all());
     ...
     * @return Pointer to the created Subscriber.
     */
    RTPS_DllAPI Subscriber* create_subscriber_with_profile(const std::string& profile_name,
     ...
     */
    RTPS_DllAPI Topic* create_topic(const std::string& topic_name, const std::string& type_name, const TopicQos& qos,
                                    TopicListener* listener = nullptr, const StatusMask& mask = StatusMask::all());
     ...
     * @return Pointer to the created Topic.
     */
    RTPS_DllAPI Topic* create_topic_with_profile(const std::string& topic_name, const std::string& type_name,
     ...
     */
    RTPS_DllAPI ContentFilteredTopic* create_contentfilteredtopic(const std::string& name, Topic* related_topic,
     ...
     */
    RTPS_DllAPI ReturnCode_t delete_contentfilteredtopic(const ContentFilteredTopic* a_contentfilteredtopic);
     ...
     */
    RTPS_DllAPI MultiTopic* create_multitopic(const std::string& name, const std::string& type_name,
     ...
     */
    RTPS_DllAPI Topic* find_topic(const std::string& topic_name, const fastrtps::Duration_t& timeout);
     ...
     */
    RTPS_DllAPI TopicDescription* lookup_topicdescription(const std::string& topic_name) const;
     ...
     * @return RETCODE_OK if the profile exists. RETCODE_BAD_PARAMETER otherwise.
     */
    RTPS_DllAPI ReturnCode_t get_topic_qos_from_profile(const std::string& profile_name, TopicQos& qos) const;
     ...
     * @warning Not supported yet. Currently returns RETCODE_UNSUPPORTED
     */
    RTPS_DllAPI ReturnCode_t get_discovered_participants(std::vector<InstanceHandle_t>& participant_handles) const;
     ...
     * @warning Not supported yet. Currently returns RETCODE_UNSUPPORTED
     */
    RTPS_DllAPI ReturnCode_t get_discovered_participant_data(builtin::ParticipantBuiltinTopicData& participant_data,
     ...
     * @warning Not supported yet. Currently returns RETCODE_UNSUPPORTED
     */
    RTPS_DllAPI ReturnCode_t get_discovered_topics(std::vector<InstanceHandle_t>& topic_handles) const;
     ...
     * @warning Not supported yet. Currently returns RETCODE_UNSUPPORTED
     */
    RTPS_DllAPI ReturnCode_t get_discovered_topic_data(builtin::TopicBuiltinTopicData& topic_data,
     ...
     * @return RETCODE_OK
     */
    RTPS_DllAPI ReturnCode_t get_current_time(fastrtps::Time_t& current_time) const;
     ...
     */
    RTPS_DllAPI ReturnCode_t register_type(TypeSupport type, const std::string& type_name);
     ...
     */
    RTPS_DllAPI bool new_remote_endpoint_discovered(const fastrtps::rtps::GUID_t& partguid, uint16_t userId,
     ...
     * @return SampleIdentity
     */
    RTPS_DllAPI fastrtps::rtps::SampleIdentity get_types(const fastrtps::types::TypeIdentifierSeq& in) const;
     ...
     */
    RTPS_DllAPI ReturnCode_t register_content_filter_factory(const char* filter_class_name,
     ...
     */
    RTPS_DllAPI IContentFilterFactory* lookup_content_filter_factory(const char* filter_class_name);
     ...
     */
    RTPS_DllAPI ReturnCode_t unregister_content_filter_factory(const char* filter_class_name);
