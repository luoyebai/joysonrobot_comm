...
class DdsDynamicFactory {
   public:
    ...
    static void AddPrimitiveMember(TypeBuilderRef builder, const std::string& name, fdds::dds::TypeKind tk,
                                   uint32_t id = 0);

    static void AddSequenceMember(TypeBuilderRef builder, const std::string& name, fdds::dds::TypeKind elem_kind,
                                  uint32_t bound = DDS_SEQUENCE_BOUND_SIZE, uint32_t id = 0);

    static void AddSequenceMember(TypeBuilderRef builder, const std::string& name, TypeRef elem_type,
                                  uint32_t bound = DDS_SEQUENCE_BOUND_SIZE, uint32_t id = 0);

    static void AddArrayMember(TypeBuilderRef builder, const std::string& name, fdds::dds::TypeKind elem_kind,
                               const std::vector<uint32_t>& dims, uint32_t id = 0);

    static void AddArrayMember(TypeBuilderRef builder, const std::string& name, TypeRef elem_type,
                               const std::vector<uint32_t>& dims, uint32_t id = 0);

    static void AddCustomMember(TypeBuilderRef builder, const std::string& name, TypeRef type, uint32_t id = 0);
    ...
    static TypeBuilderRef CloneTypeWithRos2(TypeRef type);

    static TypeBuilderRef ParserTypeFromIdlWithRos2(const std::string& idl_path, const std::string& type_name,
                                                    const std::string& include_dir = "");

    static void PrintTypeInfo(TypeRef type, const std::string& mem_name = "", size_t indent = 0, int32_t id = -1);

    static std::string IdlSerialize(TypeRef type);

    static DataRef ParseFromJson(const nlohmann::json& data_json, TypeRef type);

    static nlohmann::json ToJson(DataRef data);
};
