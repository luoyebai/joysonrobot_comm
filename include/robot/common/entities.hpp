...
class Orientation {
   public:
    Orientation() = default;
    Orientation(float roll, float pitch, float yaw) : roll_(roll), pitch_(pitch), yaw_(yaw) {}
...
class Posture {
   public:
    Posture() = default;
    Posture(const Position& pos, const Orientation& orient) : position_(pos), orientation_(orient) {}
